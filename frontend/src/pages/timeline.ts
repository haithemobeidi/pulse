 ...
 * Timeline Page
 *
 * Shows chronological view of system snapshots and issues.
 * Helps visualize when issues occurred relative to system changes.
 */
...
export async function initTimeline() {
  ...
  try {
    // Get snapshots and issues in parallel
    ...
    timelineItems.forEach((item) => {
      if (item.type === 'issue') {
        const issue = item.data as api.Issue;
        html += `
        ...
        const snapshot = item.data as api.Snapshot;
        html += `
          <div class="timeline-item">
            <div class="timeline-content">
              ...
              ${
                snapshot.notes
                  ? `<div class="timeline-desc">${escapeHtml(snapshot.notes)}</div>`
                  : ''
              }
            </div>
          </div>
...
function formatSnapshotType(type: string): string {
